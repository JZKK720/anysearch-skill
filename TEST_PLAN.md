...
| 2 | 搜索 "AAPL"，domain=finance, sub_domain=finance.quote，`--sdp ticker=AAPL`，返回 2 条 | `## Search Results` |
| 3 | 搜索 "latest trends"，domain=finance, sub_domain=finance.quote，`--sdp ticker=TSLA,period=2025`，返回 2 条 | `## Search Results` |
| 4 | 搜索 "MSFT"，domain=finance, sub_domain=finance.quote，`-p ticker=MSFT`，返回 2 条 | `## Search Results` |
...
| # | 需求 | 预期结果第一行 |
|---|---|---|
| 6 | 搜索 "AAPL"，domain=finance, sub_domain=finance.quote，`--sub_domain_params '{"ticker":"AAPL"}'`（JSON 向后兼容） | `## Search Results` |
| 7 | 搜索 "AAPL"，domain=finance, sub_domain=finance.quote，`--sub_domain_params '{ticker:AAPL}'`（模拟 PowerShell 剥引号退化格式） | `## Search Results` |
...
| # | 需求 | 预期结果第一行 |
|---|---|---|
| 10 | batch_search：`--query "AAPL stock" --query "MSFT revenue"`，共享 `--domain finance --sub_domain finance.quote`，不加 per-item 参数 | `## Query 1:` |
| 11 | batch_search 用 `@文件` 引用 JSON 文件。文件内容：`[{"query":"AAPL earnings","sub_domain_params":"ticker=AAPL"},{"query":"MSFT revenue","sub_domain_params":"ticker=MSFT"}]`。共享 `--domain finance --sub_domain finance.quote` | `## Query 1:` |
...
| # | 需求 | 预期结果第一行 |
|---|---|---|
| 12 | batch_search 用 `@文件` 引用 JSON 文件。文件内容：`[{"query":"quantum computing"},{"query":"AAPL stock","domain":"finance","sub_domain":"finance.quote","sub_domain_params":"ticker=AAPL"}]`。**不加共享参数** | `## Query 1:` |
| 13 | batch_search 直接传内联 JSON 数组：`[{"query":"test general"},{"query":"AAPL","domain":"finance","sub_domain":"finance.quote"}]` | `## Query 1:` |
| 14 | batch_search 传 mangled JSON（无引号 key，模拟 PowerShell 剥引号）：`[{query:test general},{query:AAPL,domain:finance,sub_domain:finance.quote}]` | `## Query 1:` |
